...
@description:
Inference of the anatomy classification experiment.
...
class Inference:
    def __init__(self, configuration):
        ...
        self.data_loader = DataLoaderCustom(self.dataset, self.data_path, self.fold, self.shots_per_class,
                                            self.image_dim, self.batch_size)
    ...
    def run_inference(self):
        ...
        for set, set_loader in zip(['train', 'val', 'test'], [self.train_loader, self.val_loader, self.test_loader]):
            ...
            with torch.no_grad():
                for i, (X, Y, _) in enumerate(set_loader):
...
def main(configuration):
    ...
    wandb.init(project="exp_classification_content_inference", config=configuration)
