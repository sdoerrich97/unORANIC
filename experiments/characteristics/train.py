...
@description:
Training of the image characteristics classification experiment.
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.starting_epoch, self.epochs + 1):
            ...
            for i, (_, X, Y, _) in enumerate(self.train_loader):
            ...
            with torch.no_grad():
                ...
                for i, (_, X_val, Y_val, _) in enumerate(self.val_loader):
                    # Push the next input batch to the GPU
...
def main(training_configuration=None):
    ...
    if training_configuration is not None:
        if training_configuration["resume_training"]["resume"]:
            ...
            wandb.init(project="exp_classification_attribute", id=training_configuration["resume_training"]["wandb_id"],
                       resume="allow", config=training_configuration)

        else:
            # Initialize a weights & biases project for a training run with the given training configuration
            wandb.init(project="exp_classification_attribute", config=training_configuration)

    # Run the hyperparameter optimization run
    else:
        # Initialize a weights & biases project for a hyperparameter optimization run
        wandb.init(project="sweep_exp_classification_attribute")
...
if __name__ == '__main__':
    ...
    with open(config_file, 'r') as f:
        ...
        if sweep:
            # Configure the sweep
            sweep_id = wandb.sweep(sweep=config['hyperparameter_tuning'], project="sweep_exp_classification_attribute")
