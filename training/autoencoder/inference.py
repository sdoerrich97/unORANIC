        ...
        for set, set_loader in zip(['train', 'val', 'test'], [self.train_loader, self.val_loader, self.test_loader]):
            ...
            with torch.no_grad():
                for i, (X_orig, _, X, X_name) in enumerate(set_loader):
                    ...
                    # Calculate the loss of the current sample
                    ssim_sample = self.ssim(X_hat, X)
                    mse_sample = self.mse(X_hat, X)

                    # Peak Signal-to-Noise Ratio (PSNR)
                    if not mse_sample < 1e-15:
                        psnr_sample = 20 * torch.log10(torch.tensor(1).to(mse_sample.device)) - 10 * torch.log10(mse_sample)

                    else:
                        psnr_sample = torch.tensor(50).to(mse_sample.device)

                    # Add the loss, psnr and ssim to the overall values for this epoch
                    ssim += ssim_sample.cpu()
                    mse += mse_sample.cpu()
                    psnr += psnr_sample.cpu()
                    ...
                    print(f"\t\t\tSSIM: {ssim_sample}")
                    print(f"\t\t\tMSE: {mse_sample}")
                    print(f"\t\t\tPSNR: {psnr_sample}")
            ...
            # Average the loss, and psnr over the number of test samples
            ssim /= nr_samples
            mse /= nr_samples
            psnr /= nr_samples
...
def main(configuration):
    ...
    wandb.init(project="pretrain_content_encoder_decoder_inference", config=configuration)
